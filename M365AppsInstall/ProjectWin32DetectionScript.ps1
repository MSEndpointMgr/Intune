...
Write-LogEntry -Value "Start Project Install detection logic" -Severity 1
$RegistryKeys = Get-ChildItem -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall"
$M365Apps = "Microsoft Project"
$M365AppsCheck = $RegistryKeys | Where-Object { $_.GetValue("DisplayName") -match $M365Apps }
if ($M365AppsCheck) {
    Write-LogEntry -Value "Project detected OK" -Severity 1
    Write-Output "Project Detected"
	Exit 0
   }
else {
    Write-LogEntry -Value "Project not detected" -Severity 2
    Exit 1
}